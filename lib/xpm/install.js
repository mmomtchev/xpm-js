...
export class Install extends CliCommand {
  ...
  async doRun (args) {
    ...
    if (args.length === 0) {
      ...
      // Check for precompiled binaries for the top-level package
      if (packageJson.xpack.binaries) {
        log.verbose(`package has precompiled binaries, trying to install for ${xpack.getPlatformKey()}`)
        const destination = path.join(config.cwd, packageJson.xpack.binaries.destination)
        const present = !!(await fsPromises.stat(destination).catch(() => false));
        if (present) {
          log.verbose(`binaries already present in ${destination}`);
        }

        if (!present || config.doForce) {
          const cacheFolderPath = this.context.globalConfig.cacheFolderPath
          await xpack.downloadBinaries({
            packagePath: config.cwd,
            packageTmpPath: config.cwd,
            cacheFolderPath
          }).catch((e) => log.warn(`binaries not installed: ${e}`))
        }
      }
    } else {
  ...
  async installPackageInXpack ({
    ...
    if (destJson) {
      ...
      // Destination is not an xPack, may be a custom folder, or even a file.
      try {
    ...
    if (xpack.isXpack(globalJson)) {
      ...
      if (xpack.isBinaryXpack(globalJson)) {
        ...
        // By default, binary xPacks go to devDependencies.
      ...
      if (!this.policies.shareNpmDependencies) {
        throw new CliError(`${pack} is not an xPack, use npm to install it`)
      }
  ...
  async downloadAndProcessOneDependency ({
    ...
    if (!globalJson.xpack) {
      if (log.isVerbose()) {
        ...
        log.trace(`'${packFullName}' not an xPack,` +
          ' already installed by npm')
      }
  ...
  async addFolderLinkToGlobalRepo ({
    ...
    await makeDir(path.dirname(linkPath))
  ...
  async pacoteExtractPackage ({
    ...
    if (!json || !json.xpack) {
      if (!this.policies.shareNpmDependencies) {
        ...
        await deleteAsync(destinationTmpFolderPath, { force: true })
        throw new CliError(
          `${packFullName} is not an xPack, use npm to install it`)
      }
