  ...
  async doRun (args) {
    ...
    if (args.length === 0) {
      ...
      if (packageJson.xpack.binaries) {
        log.verbose('package has precompiled binaries, ' +
          `trying to install for ${xpack.getPlatformKey()}`)
        const destination = path.join(config.cwd,
          packageJson.xpack.binaries.destination)
        const present =
          !!(await fsPromises.stat(destination).catch(() => false))
        if (present) {
