...
export class Install extends CliCommand {
  ...
  async installPackageInXpack ({
    ...
    if (config.configurationName) {
      ...
      const plugins = xpack.packageJson.xpack.liquidPlugins
      const liquidEngine = new XpmLiquid(log, xpack.packagePath, plugins)
      let liquidMap
      try {
  ...
  async collectDependency ({
    ...
    if (dependency.platforms &&
      ...
      const platformKey = xpack.getPlatformKey()
      const platformKeysArray = dependency.platforms.toLowerCase().split(',')
        .map(string => string.toLowerCase().trim())
      if (!platformKeysArray.includes(platformKey)) {
        log.info(`${isDev ? 'devDependency' : 'dependency'} '${key}' ` +
