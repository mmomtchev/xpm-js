...
export class Xpack {
  ...
  async readPackageJson () {
    ...
    this.packagePath = path.join(this.xpackPath, 'package.json')
    log.trace(`filePath: '${this.packagePath}'`)
    try {
      const fileContent = await fsPromises.readFile(this.packagePath)
    ...
    const includes = this.getIncludes(this.packageJson.xpack.include)
    for (const inc of includes) {
      await this.processInclude(inc, this.packagePath)
    }
    if (this.packageJson.xpack.include) {
      delete this.packageJson.xpack.include
    }
  ...
  getIncludes (inc) {
    if (typeof inc === 'string') {
      ...
      throw new CliErrorInput('include must be a string or an array')
    }
  }

  /**
   * Including rules:
   * - Non-array values from including files override values
   *   from included files
   * - Array values from including files are appended to array
   *   values from included files
   * - Non-array values from multiple includes are overwritten
   *   with the last include
   * - Array values from multiple includes are concatenated in the
   *   including order
   */

  async processInclude (inc, root) {
    const reverseArrayMerge = (destinationArray, sourceArray, options) =>
      destinationArray.concat(sourceArray)

    const plugins = this.packageJson.xpack.liquidPlugins
    const liquidEngine = new XpmLiquid(this.log, this.packagePath, plugins)
    try {
      const liquidMap = liquidEngine.prepareMap(this.packageJson)
      inc = await liquidEngine.performSubstitutions(inc, liquidMap)
    } catch (err) {
      this.log.trace(util.inspect(err))
    ...
    const filePath = resolve(inc)
    this.log.trace(`including ${filePath}`)
    let fileData
    try {
      ...
      this.log.trace(util.inspect(err))
      throw new CliErrorInput(
        `included file '${filePath}' not found or malformed`)
    }
    this.packageJson.xpack = deepmerge(fileData, this.packageJson.xpack, {
      arrayMerge: reverseArrayMerge
    })
    const includes = this.getIncludes(fileData.include).reverse()
    for (const inc of includes) {
      await this.processInclude(inc, filePath)
    }
  }
